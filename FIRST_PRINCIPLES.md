...
> **Status**: v40 (2026-06-16). The framework is itself a feedback machine — refinements expected as the world signals. See `CHANGELOG.md` for evolution history.
...
### Self-regulating agentic execution — budget-aware, objective-anchored, compounding 🤖
An agent on a long-running objective should *regulate itself against an explicit budget and a named objective*: plan within the budget, monitor spend against the plan, adapt dynamically to both the budget and the company's live context, and **compound competence as it goes** — skills deepen with use, raising budget-efficiency each cycle (#8). The human sets budget + objective and approves at the seams (#15: wisdom directs, intelligence executes); the agent self-regulates *within* the envelope (*Clarity Over Gates* — the budget is the gate; the optimization inside it is the agent's). One bet, many instances: **the system improving itself** (surface failures from live telemetry → classify against these principles → auto-resolve unless a genuine conflict, a decision the operator must make, or missing architecture) is the *same shape* as **an agent running a marketing / SEO / ad campaign** (plan the spend, watch it against budget, adapt to the company, improve each cycle). Generalizes the *agent-run compounding allocator* mutation under #8 from capital to *any* bounded resource — tokens, ad spend, attention. Endgame: a user-facing dial — allocate % of revenue across fixes / R&D / marketing / payouts — and watch the company self-grow within the envelope.

*Could be wrong if* agents can't yet self-regulate reliably enough (an un-watched budget overruns or the objective drifts — then the envelope needs tighter human gates, not looser); or 'compounds with use' fails (skills plateau); or budget-as-the-only-gate is too coarse for high-consequence spend (some actions need a hard per-action approval regardless of remaining budget — pairs with *Agent-Native Surface*'s grain-not-click control).

*Alternatives*: human approves every spend (doesn't scale to agent ops); fixed per-task budgets (no dynamic adaptation); unbounded autonomy (no envelope — the failure this rejects); single global budget (can't express the per-objective % dial).
