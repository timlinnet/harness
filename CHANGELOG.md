...
**📐 New Heuristic `Generalize on adoption, not on flag — multi-tenant by default once a capability survives soak`** + a one-clause refinement to *Generative Gap Resolution* rung-3. For a multi-tenant/multi-company operator, a capability built for one tenant should be **multi-tenant-ready from line one** (parameterized seam) but generalized to all tenants **only on a by-construction adoption trigger** — soak cleared on real data + promotion off the sandbox + no open finding — **never on the act of building, never on the operator flagging it**. The split that dissolves the trap: **build-shape** (always multi-tenant-ready, cheap) vs **ship-event** (adoption-gated). Two hard invariants: **propagate the mechanism, never the payload** (per-tenant canon/content/faith-copy never crosses — fails closed), and **rides the per-tenant gates** (RLS/security/faith fire per tenant; "reachable" ≠ "live-and-firing"; autonomy-to-act is asymmetric — granting needs the security pass + soak, revoking is automatic).

Per *self-eval before admission*: it **resolves** the GGR rung-3 tension (climb only as the need recurs) rather than violating it — for a multi-tenant operator the recurrence is *structurally guaranteed*, so the generalized rung is the cheapest-that-works and the real waste is the per-tenant divergence ledger of *not* generalizing (#17). A **Heuristic, not a Principle** — contingent on the operator being multi-tenant (inert for single-tenant). Descends from *Autonomous in our wake* + *Build through the single owner* + *Guarantee by construction* + #8/#17. Origin: 2026-06-25 — operator (Tim) overruled a 24h-old "manual harvest on the 2nd demand" ICP verdict because it made *his memory* the trigger ("how am I supposed to remember what's working for one company vs another?"). Ran through Harness + CEO-premise + cross-tenant-safety + encode-it lenses. Ratified knobs (project-local): N=3 soak · external-customer surfaces auto-ship too · each tenant owns its own faith release. Project mirror: freedom-ai CLAUDE.md autodecider + `.agent/design-docs/2026-06-25-multi-tenant-by-default-on-adoption.md`; the propagation *engine* (tenant registry + soak counter + propagation event + conformance gate) is a logged follow-on build.

## v45 — 2026-06-19

**📐 New Heuristic `Invert the default: emit the action with a veto, not the question`.** Sharpens the whole anti-float family (`Resolve, don't float` / `Self-drive within the never-ask envelope` / `Fire on {decision,orchestration} shape` / `Don't float settled process`) by naming *why* it keeps getting re-violated — sometimes within hours of a fresh correction on the same rule. The cause isn't a knowledge gap; it's a **dispositional deference pull** (asking feels safe), and **you cannot out-prose a reflex** — a context-resident clause or a non-blocking recall *nudge* is by-vigilance and under-fires against a pull. Two by-construction counters: **(1)** invert the default — emit the costed self-driven *action* + a one-line **veto beat** ("proceeding unless you say otherwise"), so a *question* must justify itself and **deference is not a pass-reason**; **(2)** gate the ask at a *blocking* surface (a check that can block the turn, not a nudge). Honest reach: a text-level gate guarantees the check *happened*, not that the verdict is right (review-assisted, like `/cso`) — so (1) is the primary fix, (2) the backstop, and a *non-blocking* (2) is theater.

Per *self-eval before admission*: it **sharpens** the anti-float family (doesn't parallel it) by naming the failure *mechanism* + the by-construction counter none of the siblings state; descends from *Guarantee by construction, not by vigilance* (turned on the agent's own conversational behavior) + #17. Origin: 2026-06-19 — an operator-requested eval after the operator had to talk the agent out of **three questions the substrate already resolved** (incl. a connector-permission placement question Connector-Hierarchy + Agent-Native already answer); an adversarial pass then caught the agent's *first* proposed fix — cloning the unproven non-blocking orchestration-shape wake-nudge — as ~70% theater, which is itself the heuristic's point. Project-local mechanics (an outbound *blocking* `question-gate-stop.sh` Stop-gate replacing the ripped, soak-unproven `workflow-shape-wake.sh` nudge) stay in the host setup per the route-learnings boundary.

## v44 — 2026-06-18

**📐 New Heuristic `Self-drive within the never-ask envelope`.** The consequence×reversibility classifier governs the agent's **own work queue**, not only the product it builds. An orchestrator sitting on a backlog runs it as a self-regulating queue: re-derive live state each cycle, classify every open item by the Freedom Engine's gates (already-resolved→close · guardrail wall→human · cheap+reversible+complete→auto-run · costed/irreversible→tee-up · taste/principle→surface · ledger-lies→reconcile), execute below the consequence bar **within the walls**, and **halt only when nothing auto-runnable remains** — which *is* "run until the human is actually needed." Three by-construction constraints: hard walls that override the score (RLS/auth/migrations/deps/protected-paths/faith/trust-boundary → always human); attention as a **ceiling not a minimand** (auto-run is the result of clearing a bar, never a score-improving move); a ledger row per auto-run + side-effect actuation (merge/deploy/spend) defaulting to tee-up, graduating only after a security pass + soak. The payoff finding: the **ceiling on self-drive is the wall-set, not the human's taste** — a recurring wall blocking wanted work is the signal to graduate the gate, not loosen the classifier (the attention-gaming Goodhart trap).

Per *self-eval before admission*: contradicts nothing; it is the **orchestrator-side instance** of the Freedom Engine's own "from capital to any bounded resource (tokens, ad spend, attention)" generalization — here the bounded resource is the operator's *review attention* and the queue is the agent's *backlog*, profit-free — so it descends from that Position + #17 + the cost×reversibility *never-ask reconciliation* + *Maintain tracked state* + *Guarantee by construction*, rather than re-asserting them. Standalone heuristic (not a fold) because it names the genuinely-new slice none of its parents state: the **standing-queue shape with an explicit halt-on-the-bar condition** as the agent's operating mode. *Could be wrong if* the backlog is tiny (serial is cheaper) or every item is consequential (degenerates to surface-everything, which is correct). Origin: 2026-06-18 — operator asked whether his coding agent could "auto-run loops until I'm actually needed, with the attention equation applying to my instance too"; a first run over a real 146-item backlog (66 movable-without-the-operator) confirmed the shape + the wall-set-is-the-binding-constraint finding. FreedomOS instantiation (the `self-drive` skill + classifier + decision-ledger) stays project-local per the route-learnings boundary.

## v43 — 2026-06-18

**📐 New Heuristic `Bind to a contract, not a snapshot`.** When a component consumes another's output across a boundary that can move independently (a tool's name/shape/semantics, a generated types file, a cloned data structure, a schema), declare the binding as an explicit machine-checkable contract co-located with the consumer, and gate it both ways: a build-time check that the source still resolves to the declared shape (BINDING drift — rename/relocate/field-drop) **and** a runtime envelope that refuses-and-surfaces on stale/cloned/empty/out-of-range (VALUE drift). A binding that silently resolves against a drifted source is the most common silent-failure class in a fast-moving system, because the snapshot is internally consistent and tests assert code against their own assumptions — the gap produces silence, not an error.

Per *self-eval before admission*: contradicts no Principle; it is the cover-every-input clause of *Guarantee by construction, not by vigilance* raised from the single premise-input (the v41 *strategic-premise detection* extension) to **every data-input across a movable boundary**, and pairs with #12 (derive from source-of-truth, but make the derivation self-checking). Lands as a standalone Heuristic (an actionable two-net design rule neither parent states). *Could be wrong if* the boundary is genuinely stable (frozen vendor API / internal constant → ceremony) or consumer+producer share one end-to-end type system (compiler is the gate). Origin: 2026-06-18 — a dedicated subagent investigation (operator-requested) into how *The Freedom Engine*'s data bindings (revenue listeners, finance endpoints, MCP tools — "moving targets as the architecture evolves") stay correct; the host repo's recurring stale-resolution scar recurred across nine instances, each the same bug, because every existing conformance gate watched one artifact's internal consistency and none watched the cross-boundary binding. FreedomOS instantiation (a `bindings.ts` registry + `check-equation-bindings.sh` gate + `freshness.ts` runtime envelope) stays project-local per the route-learnings boundary.
